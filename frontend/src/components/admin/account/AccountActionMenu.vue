      ...
      >
        <div class="py-1">
          <template v-if="account">
            ...
            <button v-if="isOpenAI" @click="$emit('toggle-response-model-rewrite', account); $emit('close')" class="flex w-full items-center gap-2 px-4 py-2 text-sm hover:bg-gray-100 dark:hover:bg-dark-700">
              <Icon name="refresh" size="sm" :class="responseModelRewriteEnabled ? 'text-emerald-500' : 'text-gray-500'" />
              {{ t('admin.accounts.responseModelRewrite') }}: {{ responseModelRewriteEnabled ? t('common.enabled') : t('common.disabled') }}
            </button>
...
const emit = defineEmits(['close', 'test', 'stats', 'schedule', 'duplicate', 'reauth', 'refresh-token', 'recover-state', 'reset-quota', 'set-privacy', 'create-spark-shadow', 'toggle-response-model-rewrite'])
const { t } = useI18n()
...
const responseModelRewriteEnabled = computed(() => Boolean((props.account?.extra as Record<string, unknown> | undefined)?.openai_response_model_rewrite_enabled))
